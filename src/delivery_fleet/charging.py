...
DEFAULT_CHARGING_POWER_W = 2_000.0
...
def _edge_weight_value(graph: nx.Graph, u: NodeId, v: NodeId, weight: str) -> float:
...
def nearest_station_data(
    ...
    *,
    ...
    """One memory-efficient multi-source Dijkstra for nearest charger data."""
    ...
    while heap:
        ...
        for neighbor in graph.neighbors(node):
            ...
            if candidate < old_distance or better_tie:
                ...
                heapq.heappush(
                    heap,
                    (candidate, source_rank, next(counter), neighbor, source),
                )
...
def annotate_nearest_charging_stations(
    ...
    *,
    ...
    nx.set_node_attributes(
        graph, distances, DISTANCE_TO_NEAREST_CHARGING_STATION_M_ATTR
    )
    nx.set_node_attributes(graph, nearest, NEAREST_CHARGING_STATION_NODE_ATTR)
